...
当我们手动去 cancel 一个 context 的时候, 是需要将其从 parent 的 children 中移除的, 因为重复 close 一个 channel 会导致 panic, 而当前 context 的 children 就不用移除了, 已经 cancel 的 context 就算保留着 children 也没问题, 反正是无法重复 cancel 的, 这样也避免了多余的内存操作.
